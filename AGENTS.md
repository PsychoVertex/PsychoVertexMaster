...
- `Pipeline.py`: modal task runner used by `UnpackActiveCollection` and `BakeBatch`. It gives every task an announcement/redraw tick and supports item-level header and progress updates through `set_pipeline_detail(text, current, total)`.
...
5. Run `lightmap.unpack_active_collection` on the active direct child of `SOURCE`. It creates or replaces only that source collection's `EXPORT_STUFF/BatchN (SourceCollectionName)`, preserves unrelated batches, reuses the source's existing batch number, and excludes only that source child after success. It realizes collection instances and duplicates ordinary object hierarchies, applies generated-object modifiers, normalizes geometry scale, preserves light transforms without applying unsupported scale operations to light datablocks, corrects mirrored mesh normals, and preserves separate objects, parent empties, lights, collision helpers, and `visible_shadow`. It then optionally packs eligible meshes together in the shared `LightMap` UV space. Preparation errors either cancel atomically or may be explicitly ignored, which marks the batch as not fully import-ready.
...
- `UnpackActiveCollection.heuristicDuration` controls UVPackmaster's final-pass `heuristic_search_time` in seconds for the batch.
- `UnpackActiveCollection.packLightmaps` defaults to enabled. When disabled for testing, realization still runs but the UV packing pipeline task is omitted entirely.
